...
class StickyComponent {
  ...
  private enable = (update = false) => {
    ...
    if (left !== null && String(left)?.toLowerCase() === 'auto') {
  ...
  // Event API
  public on = (name: string, callBack: any) => EventHandlerUtil?.on(this?.element, name, callBack)

  public one = (name: string, callback: any) => EventHandlerUtil?.one(this?.element, name, callback)

  public off = (name: string) => EventHandlerUtil?.off(this?.element, name)

  public trigger = (name: string) => EventHandlerUtil?.trigger(this?.element, name)
