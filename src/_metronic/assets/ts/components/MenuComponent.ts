...
class MenuComponent {
  ...
  private _isTriggerElement = (item: HTMLElement) => this?.triggerElement === item
  ...
  private _getItemChildElement = (item: HTMLElement) => {
    ...
    if (selector) {
      ...
      const element: any = selector?.querySelector('.menu-item[data-kt-menu-trigger]')
      if (element) {
        return element
      }
    }
  ...
  private _isItemDropdownPermanent = (item: HTMLElement) =>
    this?._getItemOption(item, 'permanent') === true

  // Test if item's parent is shown
  private _isItemParentShown = (item: HTMLElement) =>
    getElementParents(item, '.menu-item?.show')?.length > 0

  // Test of it is item sub element
  private _isItemSubElement = (item: HTMLElement) => item?.classList?.contains('menu-sub')

  // Test if item has sub
  private _hasItemSub = (item: HTMLElement) =>
    item?.classList?.contains('menu-item') && item?.hasAttribute('data-kt-menu-trigger')

  // Get link element
  private _getItemLinkElement = (item: HTMLElement) => getElementChild(item, '.menu-link')
  ...
  private _hideAccordions = (item: HTMLElement) => {
    const itemsToHide: any = this?.element?.querySelectorAll('.show[data-kt-menu-trigger]')
  ...
  private _update = () => {
    const items: any = this?.element?.querySelectorAll('.menu-item[data-kt-menu-trigger]')
    items?.forEach((el: any) => this?._reset(el as HTMLElement))
  }
  ...
  // ** Public API  ** //
  ///////////////////////
  public click = (element: HTMLElement, e: Event) => this?._click(element, e)

  public link = (element: HTMLElement, e: Event) => this?._link(element, e)

  public dismiss = (element: HTMLElement, e: Event) => this?._dismiss(element, e)

  public mouseover = (element: HTMLElement, e: Event) => this?._mouseover(element, e as MouseEvent)

  public mouseout = (element: HTMLElement, e: Event) => this?._mouseout(element, e as MouseEvent)

  // General Methods
  public getItemTriggerType = (item: HTMLElement) => this?._getItemOption(item, 'trigger')

  public getItemSubType = (element: HTMLElement) => this?._getItemSubType(element)

  public show = (item: HTMLElement) => this?._show(item)

  public hide = (item: HTMLElement) => this?._hide(item)

  public reset = (item: HTMLElement) => this?._reset(item)

  public update = () => this?._update()

  public getElement = () => this?.element

  public getItemLinkElement = (item: HTMLElement) => this?._getItemLinkElement(item)

  public getItemToggleElement = (item: HTMLElement) => this?._getItemToggleElement(item)

  public getItemSubElement = (item: HTMLElement) => this?._getItemSubElement(item)

  public getItemParentElements = (item: HTMLElement) => this?._getItemParentElements(item)

  public isItemSubShown = (item: HTMLElement) => this?._isItemSubShown(item)

  public isItemParentShown = (item: HTMLElement) => this?._isItemParentShown(item)

  public getTriggerElement = () => this?.triggerElement

  public isItemDropdownPermanent = (item: HTMLElement) => this?._isItemDropdownPermanent(item)

  // Accordion Mode Methods
  public hideAccordions = (item: HTMLElement) => this?._hideAccordions(item)
  ...
  public static hideDropdowns = (skip: HTMLElement | undefined) => {
    const items: any = document?.querySelectorAll('.show.menu-dropdown[data-kt-menu-trigger]')
  ...
  public static initGlobalHandlers = () => {
    ...
    document?.addEventListener('click', (e: any) => {
      const menuItems: any = document?.querySelectorAll('.show.menu-dropdown[data-kt-menu-trigger]')
    ...
    window?.addEventListener('resize', () => {
      ...
      throttle(
        ...
        () => {
          ...
          const elements: any = document?.querySelectorAll('[data-kt-menu="true"]')
  ...
  public static createInsance = (
    ...
    const element: any = document?.body?.querySelector(selector)
    if (!element) {
      return
    }
