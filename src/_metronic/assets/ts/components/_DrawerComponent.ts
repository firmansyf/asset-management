...
class DrawerComponent {
  ...
  private _createOverlay = () => {
    if (this?._getOption('overlay') === true) {
      ...
      this?.overlayElement?.addEventListener('click', (e: any) => {
        e?.preventDefault()
        this?._hide()
      })
    }
  ...
  private _getDirection = () =>
    String(this?._getOption('direction')) === 'left' ? 'left' : 'right'
  ...
  public isShown = () => this?.shown

  public update = () => {
    this?._update()
  }

  public goElement = () => this?.element
  ...
  // Static methods
  public static hasInstace = (element: HTMLElement): boolean => DataUtil?.has(element, 'drawer')

  public static getInstance = (element: HTMLElement) => DataUtil?.get(element, 'drawer')

  public static hideAll = () => {
    const oldInstances = DataUtil?.getAllInstancesByKey('drawer')
    oldInstances?.forEach((dr: any) => {
      dr?.hide()
    })
  }
  ...
  public static handleDismiss = () => {
    ...
    DOMEventHandlerUtil?.on(document?.body, '[data-kt-drawer-dismiss="true"]', 'click', () => {
      const element = (this as any)?.closest('[data-kt-drawer="true"]')
      if (element) {
