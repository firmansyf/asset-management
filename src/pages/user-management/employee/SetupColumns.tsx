...
import {
  getSetupColumnsEmployee,
  updateSetupColumnsEmployee,
} from '@pages/user-management/redux/EmployeeCRUD'
...
let SetupColumnsEmployee: FC<any> = () => {
  const intl = useIntl()
    ...
    getSetupColumnsEmployee()
      .then(
        ({
          ...
          const renameFields: any = renameObject(fileds_columns) as never
  ...
  const saveColumns = () => {
    ...
    updateSetupColumnsEmployee({columns: {fields}})
      .then(({data: {message}}: any) => {
        navigate('/user-management/employee')
      ...
      .catch((err: any) => {
        ...
        if (data?.fields !== undefined) {
          ...
          for (const key in error) {
            ...
            ToastMessage({type: 'error', message: Array.isArray(value) ? value?.[0] : 'Error'})
          }
        } else {
    ...
    <>
      ...
      <div className='card card-custom'>
        <div className='card-footer text-end'>
          <Link to='/user-management/employee' className='btn btn-sm btn-secondary me-2'>
            {' '}
            Cancel{' '}
          </Link>
...
SetupColumnsEmployee = memo(
  SetupColumnsEmployee,
  (prev: any, next: any) => JSON.stringify(prev) === JSON.stringify(next)
)
export default SetupColumnsEmployee
