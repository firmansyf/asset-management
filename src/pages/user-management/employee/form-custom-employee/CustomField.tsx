...
let CustomField: FC<Props> = ({item, setFieldValue, currency, preference, detail, values}) => {
  const {preference: defaultPreference}: any = useSelector(
    ({preference}: any) => preference,
    shallowEqual
  )
  const required = item.is_required === 1 ? `${configClass?.label} required` : configClass?.label
  ...
  useEffect(() => {
    if (item?.element_type === 'currency') {
      const cekCurrency: any = dataCurrency?.filter(
        ({value}: any) => value === defaultPreference?.currency
      )
      if (cekCurrency?.length > 0) {
        Array.isArray(cekCurrency) && setPreferenceCurrency(cekCurrency[0])
      } else {
        Array.isArray(dataCurrency) && setPreferenceCurrency(dataCurrency[0])
      }
    }
  }, [dataCurrency, defaultPreference?.currency, item?.element_type])
          ...
          <div className='input-group input-group-solid m-0'>
            <div className='col-6'>
              <Select
                ...
                // isClearable={false}
                // components={{ Option: ({ innerProps, data }: any) =>
                //   <div {...innerProps} style={{ borderBottom: '1px solid #EBEBEB', padding: "8px 10px" }}>{ data.label }</div>
                // }}
                // getOptionValue={(option: any) => option.value }
                // getOptionLabel={(option: any) => option.value }
                onChange={({value}: any) => {
                  setFieldValue(`global_custom_fields[${item.guid}][value][code]`, value || '')
                }}
              />
            </div>
            <div className='col-6'>
        ...
        {item.element_type === 'date' && (
          <div className='input-group input-group-solid m-0'>
            ...
            <Datetime
              closeOnSelect
              inputProps={{
                'data-cy': `${item?.name?.split(' ')?.join('-')?.toLowerCase()}-value`,
                autoComplete: 'off',
                className: configClass?.form,
                name: `global_custom_fields[${item.guid}][value]`,
                placeholder: 'Enter ' + item.name,
              ...
              onChange={(e: any) => {
                ...
                setFieldValue(`global_custom_fields[${item.guid}][value]`, dt)
                setDefaultValueDate(dt)
              }}
