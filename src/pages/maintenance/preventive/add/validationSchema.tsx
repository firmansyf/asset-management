...
const Validation: FC<Props> = ({
  ...
  useEffect(() => {
    let validationShape: any = {
      name: Yup.string().required('Maintenance Preventive Name is required'),
      ...
      asset_guid: Yup.object().required('Asset by Location is required'),
    ...
    if (checkPriority?.is_required) {
      validationShape = {
        ...
        maintenance_priority_guid: Yup.mixed().test({
          name: 'maintenance_priority_guid',
          test: function () {
            const {maintenance_priority_guid} = this.parent || {}
            if (
              maintenance_priority_guid === undefined ||
              Object.keys(maintenance_priority_guid || {})?.length === 0 ||
              maintenance_priority_guid?.value === ''
            ) {
              return this.createError({
    ...
    setPreventiveSchema(WoSchema)
  }, [setPreventiveSchema, checkPriority, checkCategory, checkWorker, checkAdditionalWorker])
  return null
}
