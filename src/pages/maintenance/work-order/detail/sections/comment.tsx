...
import {getWorkOrderComment, postWorkOrderComment} from '@pages/maintenance/Service'
...
let Comment: FC<any> = (props: any) => {
  ...
  const sendComment = () => {
    ...
    const {guid, maintenance_guid}: any = data || {}
    ...
    postWorkOrderComment(params, maintenance_guid || guid)
      .then(() => {
  ...
  const getDataComment = useCallback(() => {
    ...
    if (guid) {
      getWorkOrderComment(maintenance_guid || guid || '')
        .then(({data: {data: res}}) => {
          if (res) {
            setRowComment(res)
