...
let AddFile: FC<AddFileProps> = ({setFieldValue, validation, files}) => {
    ...
    if (files) {
      files?.photos &&
        files?.photos?.map(({url, title}: any) =>
          urlToFile(`${url}?token=${token}`, title).then(({file, base64}: any) => {
            setImages(
              (prev: any) =>
                uniqBy([...prev, {dataURL: base64, file: file}], 'file.name') as never[]
            )
          })
        )
      ...
      files?.others &&
        files?.others?.map(({url, title}: any) =>
          urlToFile(`${url}?token=${token}`, title).then(({file}: any) => {
            setOtherFile((prev: any) => uniqBy([...prev, file], 'name') as never[])
          })
        )
  ...
  useEffect(() => {
    if (otherDefault && otherDefault?.length > 0) {
      const data = otherDefault?.map(({dataURL, file}: any) => {
        return {
          data: dataURL || '',
          title: file?.name || '',
        }
  ...
  }, [otherDefault, setFieldValue])

  useEffect(() => {
    if (images && images?.length > 0) {
      const data = images?.map(({dataURL, file}: any) => {
        return {
          data: dataURL || '',
          title: file?.name || '',
        }
  ...
  const onChangeUploadPhoto = (imageList: ImageListType) => {
    if (imageList?.[0]) {
      const data = imageList?.map(({dataURL, file}: any) => {
        return {
          data: dataURL || '',
          title: file?.name || '',
        }
  ...
  return (
    <div className='row'>
      <div className='col-md-4'>
        ...
        <ImageUploading value={images} onChange={onChangeUploadPhoto} maxNumber={3} multiple>
          {({
            imageList,
            onImageUpload,
            onImageUpdate,
