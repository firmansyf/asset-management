...
const CardReportDepartment: FC<any> = () => {
  ...
  const onExport = (e: any) => {
    ...
    exportMyAsset({type: e, columns: params?.join(','), report_name: 'Department Report'})
      .then(({data: res}: any) => {
  ...
  const dataDepartmentParam: any = {page, limit, keyword, orderDir, orderCol}
  const dataDepartmentQuery: any = useQuery({
    // initialData: {data: []},
    queryKey: ['getAssetDepartment', {...dataDepartmentParam}],
    queryFn: async () => {
      if (columns?.length > 0) {
        const res: any = await getAsset(dataDepartmentParam)
    ...
    <>
      ...
      {PermissionView && (
        <ModalAutomatedReport
          ...
          detail={{name: 'Departments', type: 'asset'}}
          onSubmit={() => {
...
let ReportDepartment: FC = () => {
  const intl: any = useIntl()
  return (
    <>
      <PageTitle breadcrumbs={[]}>{intl.formatMessage({id: 'MENU.REPORTS.DEPARTMENT'})}</PageTitle>
      <CardReportDepartment />
    </>
  )
}

ReportDepartment = memo(
  ReportDepartment,
