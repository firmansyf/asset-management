...
const CardReportCategoryManufacture: FC<any> = () => {
  ...
  const columns: any = [
    ...
    {header: 'Manufacture', sort: true, value: 'manufacturer_name'},
  ...
  const onExport = (e: any) => {
    ...
    exportMyAsset({type: e, columns: params?.join(','), report_name: 'Category & Manufacturer'})
      .then(({data: res}) => {
  ...
  const dataCategoryManufactureParam: any = {page, limit, keyword, orderDir, orderCol}
  const dataCategoryManufactureQuery: any = useQuery({
    // initialData: {data: []},
    queryKey: ['getAssetCategoryManufacture', {...dataCategoryManufactureParam}],
    queryFn: async () => {
      if (columns?.length > 0) {
        const res: any = await getAsset(dataCategoryManufactureParam)
  ...
  const dataCategoryManufacture: any = dataCategoryManufactureQuery?.data || []
    ...
    <>
      <div className='card card-custom card-table'>
        ...
        <div className='card-body'>
          <DataTable
            ...
            data={dataCategoryManufacture}
            loading={!dataCategoryManufactureQuery?.isFetched}
          />
        </div>
      </div>
      ...
      {PermissionView && (
        <ModalAutomatedReport
          ...
          detail={{name: 'Category & Manufacturer', type: 'asset'}}
          onSubmit={() => {
...
let ReportCategoryManufacture: FC = () => {
  const intl = useIntl()
  return (
    <>
      <PageTitle breadcrumbs={[]}>
        {intl.formatMessage({id: 'MENU.REPORTS.CATEGORY_MANUFACTURER'})}
      </PageTitle>
      <CardReportCategoryManufacture />
    </>
  )
}

ReportCategoryManufacture = memo(
  ReportCategoryManufacture,
  (prev: any, next: any) => JSON.stringify(prev) === JSON.stringify(next)
)
export default ReportCategoryManufacture
