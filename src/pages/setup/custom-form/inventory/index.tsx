...
const Index: FC<any> = () => {
  ...
  const onSave: any = () => {
    ...
    saveCustomForm({module: 'inventory', forms})
      .then(({data: {message}}: any) => {
        setLoadingBtn(false)
        setShowModalSave(false)
        ToastMessage({type: 'success', message})
        setTimeout(() => {
          navigate(id ? `/inventory/add?id=${id}` : '/inventory/add')
        }, 100)
      })
      .catch((err: any) => {
        setLoadingBtn(false)
  ...
  const onDeleteGroup: any = () => {
    ...
    const forms: Array<PutDataTypes> = [...arrGroup, ...arrForms]
    saveCustomForm({module: 'inventory', forms}).then(() => {
  ...
  return (
    <>
      <PageTitle>{intl.formatMessage({id: 'MENU.SETUP.CUSTOM_FORM.INVENTORY'})}</PageTitle>
      <CustomForm
        data={data}
        onChange={setData}
      ...
      <Alert
        ...
        body={
          <div>
            Are you sure want to save <strong>&quot;Custom Inventory Form&quot;</strong> ?
          </div>
        }
        type={'save'}
        key={'save'}
        title={`Save Inventory Form`}
        confirmLabel={'Save'}
        onConfirm={onSave}
