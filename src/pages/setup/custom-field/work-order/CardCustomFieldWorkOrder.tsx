...
let CardCustomFieldWorkOrder: FC<any> = ({
  onDelete,
  setReloadData,
  reloadData,
  checked,
  ...
  const label: string = 'Custom Field Maintenance'
  const section_type: string = 'maintenance'
  ...
  const params: any = {
    ...
    orderCol,
    keyword,
    filter: {section_type: 'maintenance'},
  }
  const CustomFieldsMaintenanceQuery: any = useQuery({
    queryKey: ['getDataCFMaintenance', {...params, reloadData, setReloadData}],
    queryFn: async () => {
  ...
  const dataCFWorkOrder: any = CustomFieldsMaintenanceQuery?.data || []
    ...
    <>
      <div className='card card-custom card-table'>
        ...
        <div className='card-body'>
          <DataTable
            page={page}
            loading={!CustomFieldsMaintenanceQuery?.isFetched}
            limit={limit}
            total={totalPage}
            data={dataCFWorkOrder}
            columns={columns}
            ...
            customEmptyTable={'No Maintenance Custom Field Added'}
          />
        </div>
      </div>
...
CardCustomFieldWorkOrder = memo(
  CardCustomFieldWorkOrder,
  (prev: any, next: any) => JSON.stringify(prev) === JSON.stringify(next)
)
export default CardCustomFieldWorkOrder
