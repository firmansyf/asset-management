...
let CardCustomFieldEmployee: FC<any> = ({
  reloadData,
  setReloadData,
  checked,
  setChecked,
  ...
  const label: string = 'Custom Field Employee'
  const section_type: string = 'employee'
  ...
  const confirmDeleteBulk = useCallback(() => {
    const guids: any = checked?.map(({guid}: any) => guid)
  ...
  }, [reloadData, setReloadData, checked, setChecked])
  ...
  const params: any = {
    ...
    orderCol,
    keyword,
    filter: {section_type: 'employee'},
  }
  const CustomFieldsEmployeeQuery: any = useQuery({
    queryKey: ['getDataCFEmployee', {...params, reloadData}],
    queryFn: async () => {
      ...
      setTotalPage(total)
      setPageFrom(from)
      const dataRes: any = results?.data?.map((item: any) => {
  ...
  const dataCFEmployee: any = CustomFieldsEmployeeQuery?.data || []
    ...
    <>
      <div className='card card-custom card-table'>
        ...
        <div className='card-body'>
          <DataTable
            page={page}
            loading={!CustomFieldsEmployeeQuery?.isFetched}
            limit={limit}
            total={totalPage}
            data={dataCFEmployee}
            columns={columns}
            ...
            customEmptyTable={'No Employee Custom Field Added'}
          />
        </div>
      </div>
...
CardCustomFieldEmployee = memo(
  CardCustomFieldEmployee,
  (prev: any, next: any) => JSON.stringify(prev) === JSON.stringify(next)
)
export default CardCustomFieldEmployee
