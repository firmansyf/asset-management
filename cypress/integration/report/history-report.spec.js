...
describe('Report History Report, Testing...', () => {
    beforeEach( () => { 
        cy.visit('reports/history-reports')
      }) 

    it('Fetch Report History Reports', () => {
    ...
    it('Test Button Actions', () => {
      cy.get('.dropdown').find('button[type="button"]').should('have.class', 'dropdown-toggle').click({force: true, multiple: true})
    })
    ...
    it('Test Button Actions Automated Report', () => {
      ...
      cy.get('form').within(() => {
        ...
        cy.get('.modal-body').within(() => {
          cy.get('.report-name').find('input[name*="name"]').should('have.value', 'History')
          cy.get('.report-team').find('select[name*="team_guid"]').select('testing (0)')
          cy.get('.report-frequency').within(() => {
            cy.get('input[name*="frequency_value"]').click({multiple: true})
          })
        })
        cy.get('.modal-footer').find('button[type=submit]').contains('Add').click({force: true})
      })
    })
