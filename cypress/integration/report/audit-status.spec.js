beforeEach(cy.login)
describe('Report Audit Status, Testing...', () => {
  beforeEach( () => { 
    cy.visit('reports/audit-status')
    cy.intercept(`**/report/**`).as('fecthData')
    cy.wait('@fecthData')
  })
    it('Fetch Report Audit Status', () => {
    ...
    it('Test Button Actions Automated Report', () => {
      ...
      cy.get('form').within(() => {
        ...
        cy.get('.modal-body').within(() => {
          cy.get('.report-name').find('input[name*="name"]').should('have.value', 'Audit Status')
