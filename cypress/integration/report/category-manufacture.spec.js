beforeEach(cy.login)
describe('Report Category & Manufacturer, Testing...', () => {
  beforeEach( () => { 
    cy.visit('reports/category-manufacturer')
  })
    it('Fetch Repor tCategory & Manufacturer', () => {
    ...
    it('Search', () => {
        ...
        cy.intercept('GET', `${Cypress.env('api')}report/**`)
    })
    ...
    it('Test Button Actions Automated Report', () => {
      ...
      cy.get('form').within(() => {
        ...
        cy.get('.modal-body').within(() => {
          cy.get('.report-name').find('input[name*="name"]').should('have.value', 'Category & Manufacturer')
